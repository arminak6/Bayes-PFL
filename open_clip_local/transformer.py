...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        return self.attn(
            q_x, k_x, v_x, need_weights=True, attn_mask=attn_mask  # need_weights=False,
        )
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.arch, self.attn_strategy, self.gaussian_std = 'reduced', 'naclip', 5.0
    ...
    def custom_attn(self, attn_layer, x, return_attn=False, with_attn=False, n_patches=None):
        ...
        if self.attn_strategy in ['naclip', 'nonly', 'gav']:
            ...
            if addition is None:
                ...
                window = Transformer.gaussian_window(*window_size, std=self.gaussian_std)
                addition = Transformer.get_attention_addition(*n_patches, window).unsqueeze(0).to(x.dtype).to(x.device)
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9], attn_mask: Optional[torch.Tensor] = None):
        idx = 0
        out_tokens = []
        out_attn = []
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                elif idx == len(self.resblocks) and x.size(0) != 77:
                    # x = self.custom_attn(r.attn, r.ln_1(x), n_patches=(16, 16))
                    # x = self.custom_attn(r.attn, r.ln_1(x), n_patches=(32, 32))
                    x = self.custom_attn(r.attn, r.ln_1(x))
                else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list):
        ...
        patch_tokens = [patch[:,1:,:] for patch in patch_tokens]
        
        return pooled, tokens @ self.proj, patch_tokens
